    ...
    <div className="space-y-6">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2">
          {savedGuides.map((g) => (
            <button
              key={g.id}
              onClick={() => viewGuide(g)}
              className={`glass-card rounded-xl px-3 py-2 text-xs whitespace-nowrap shrink-0 transition-colors ${
      ...
      {mode === "input" && (
        <div className="space-y-4">
          ...
          >
            <p className="font-serif text-sm text-theme-text-muted mb-3">
              Drag & drop a text file or PDF to upload
            </p>
            <label className="inline-block glass rounded-xl px-5 py-2 shadow-glass text-sm font-medium text-theme-text cursor-pointer hover:scale-[1.02] transition-transform">
              Upload File
              <input
                type="file"
          ...
          <div className="glass-card rounded-2xl p-4">
            <p className="text-xs text-theme-text-muted mb-2">Or paste/write your notes:</p>
            <textarea
              value={inputText}
              onChange={(e) => setInputText(e.target.value)}
              className="w-full min-h-[200px] rounded-xl px-4 py-3 bg-theme-bg border border-theme-accent/20 text-theme-text text-sm placeholder:text-theme-text-muted/50 focus:outline-none focus:border-theme-accent resize-y"
          ...
          <div className="grid grid-cols-3 gap-2">
            {FORMAT_OPTIONS.map((opt) => (
              ...
              >
                <span className="block font-serif text-xs font-medium text-theme-text">
                  {opt.label}
                </span>
          ...
          <button
            ...
            className="w-full glass rounded-2xl px-6 py-3 shadow-glass font-serif text-sm font-medium text-theme-text hover:scale-[1.01] transition-transform disabled:opacity-50"
          >
        ...
        <div className="space-y-4">
          ...
          <div className="glass-card rounded-2xl p-4 space-y-3">
            <button
              ...
              className="glass rounded-xl px-4 py-2 shadow-glass text-xs font-medium text-theme-text hover:scale-[1.02] transition-transform disabled:opacity-50"
            >
                ...
                {flashcardsSuggested.map((c, i) => (
                  <li key={i} className="flex items-center justify-between gap-2 text-xs">
                    ...
                    <button
                      ...
                      className="shrink-0 px-2 py-1 rounded-lg bg-theme-accent/10 text-theme-accent text-[10px] font-medium hover:bg-theme-accent/20"
                    >
                      Add
                    </button>
                  </li>
